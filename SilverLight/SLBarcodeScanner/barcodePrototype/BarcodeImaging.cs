...
public class Bitmap
{
    ...
    public int BytesPerPixel
    {
        get
        {
            switch (PixelFormat)
            {
                case global::PixelFormat.Format24bppRgb:
                    return 3;

                case global::PixelFormat.Format32bppArgb:
                    return 4;

                case global::PixelFormat.Format32bppRgb:
                    return 4;
            }
            return 1;
        }
    ...
    public BitmapData LockBits(Rect rect)
    {
        BitmapData bmpd = new BitmapData();
        bmpd.Width = (int)rect.Width;
        bmpd.Height = (int)rect.Height;
        bmpd.Offset = (int)(Width * rect.Y + rect.X) * BytesPerPixel;
        bmpd.Buffer = Buffer;
        bmpd.Stride = Width * BytesPerPixel;
        return bmpd;
    }
}

public class BitmapData : Bitmap
{
    public int Stride
    {
        get;
...
public class BarcodeImaging
{
    ...
    public static void FullScanPage(ref List<string> CodesRead, Bitmap bmp, int numscans, ref long offset)
    {
        ScanPage(ref CodesRead, bmp, numscans, ScanDirection.Vertical, FullScanBarcodeTypes, ref offset);
        ScanPage(ref CodesRead, bmp, numscans, ScanDirection.Horizontal, FullScanBarcodeTypes, ref offset);
    }
    ...
    public static void ScanPage(ref List<string> CodesRead, Bitmap bmp, int numscans, ScanDirection direction, BarcodeType types, ref long offset)
    {
        int iHeight, iWidth;
    ...
    private static histogramResult verticalHistogram(Bitmap bmp, int start, int end, ScanDirection direction)
    {
        ...
        if (direction == ScanDirection.Horizontal)
        {
            bmData = bmp.LockBits(new Rect(start, 0, end - start, bmp.Height));
            xMax = bmData.Height;
            yMax = end - start;
        }
        else
        {
            bmData = bmp.LockBits(new Rect(0, start, bmp.Width, end - start));
            xMax = bmp.Width;
            yMax = bmData.Height;
        }
        ...
        //unsafe
        {
            ...
            for (int y = 0; y < yMax; ++y)
            {
                ...
                {
                    if (direction == ScanDirection.Horizontal)
                        vertSum[x] += getpixelbrightness(p, offset, pf, stride, y, x);
                    else
                        vertSum[x] += getpixelbrightness(p, offset, pf, stride, x, y);
                }
            }
        }
    ...
    {
        Dictionary<int, int> hdLightBars = new Dictionary<int, int>();
        Dictionary<int, int> hdDarkBars = new Dictionary<int, int>();
        ...
        for (int i = iStart + 1; i < iEnd; i++)
        {
            ...
            if (bDark != bDarkBar)
            {
                ...
                if (bDarkBar)
                {
                    if (!hdDarkBars.ContainsKey(iBarWidth))
                ...
                {
                    if (!hdLightBars.ContainsKey(iBarWidth))
    ...
    private static void CalcNarrowBarWidth(Dictionary<int, int> hdBarWidths, out float fNarrowBarWidth, out float fWiderBarWidth)
    {
    ...
    private static void FindBarcodeZones(ref histogramResult hist)
    {
        ...
        {
            List<BarcodeZone> alBarcodeZones = new List<BarcodeZone>();
            bool bDarkBar = (hist.histogram[0] <= hist.threshold);
            int iBarStart = 0;
            int iZoneStart = -1;
            int iZoneEnd = -1;
            ...
            if (alBarcodeZones.Count > 0)
                hist.zones = (BarcodeZone[])alBarcodeZones.ToArray();
            else
                hist.zones = null;
        }
    }
    ...
    /// <returns></returns>
    private static ushort getpixelbrightness(byte[] p, int offset, PixelFormat pf, int stride, int x, int y)
    {
        ushort uBrightness = 0;
        switch (pf)
        {
            case PixelFormat.Format1bppIndexed:
                byte b = p[offset + (y * stride) + (x >> 3)];
                if (((b << (x % 8)) & 128) != 0)
                    uBrightness = 255;
                break;
