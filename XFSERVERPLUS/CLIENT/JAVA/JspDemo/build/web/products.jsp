...
<%
	...
	String SelectedGroup = ((xfpldemo.Product_group)ProductGroups.get(0)).getName();
	...
	if ((session.getAttribute("LASTGROUP")!=null) && (session.getAttribute("PRODUCTS")!=null)) {
		...
		ProductCount = synergy.GetProducts(SelectedGroup,Products);
		if (ProductCount > 0) {
...
<%
	...
	{
		out.print("          <option value='" + ((xfpldemo.Product_group)ProductGroups.get(cnt)).getName() + "'");
		if (SelectedGroup.equals(((xfpldemo.Product_group)ProductGroups.get(cnt)).getName())) {
			out.print(" selected");
		}
		out.println( ">"+((xfpldemo.Product_group)ProductGroups.get(cnt)).getDescription()+"</option>");
	}
...
<%
	  ...
	  for (cnt=0; cnt<ProductCount; cnt++) {
		...
		xfpldemo.Product CurrentProduct = (xfpldemo.Product)Products.get(cnt);
