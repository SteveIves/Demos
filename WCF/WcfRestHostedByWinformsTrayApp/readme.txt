...
Title:          WcfRestHostedByWinformsTrayApp

Description:    An example of self-hosting a WCF REST service inside a Windows
                forms desktop application that runs in the Windows system tray.
				Doing so would allow external applications (including apps
				running on other systems) to interact with the Windows app.

				Although this example code is configured to run as a .NET app,
				the app was originally developed to allow code on OpenVMS to
				be able to interact with Windows functionality on the same
				Windows desktop as the OpenVMS users terminal emulator is
				running.
...
TrayListenerApp Project
-----------------------
This project contains a simple Windows Forms desktop application that is setup
to run in the Windows system tray. The application hosts (on a seperate thread
to the main UI thread) an instance of a WCF REST web service. This allows
external applications to send "messages" to the application in order to instruct
...
In its default implementation the WCF service has a single operation defined.
...
http://localhost:8088/LocalAppService/message/TEST
...
------  ------  ----------------------------
PING            Tests that the service is running. The service should respond
                by returning the message PONG!
BROWSER Web URL Causes a web browser to launch and display the requested URL.

TestApp Project
---------------
This project conains a traditional Synergy application that implements a simple
client to the WCF REST service that is hosted by the Windows Forms application.
The code could be executed on Unix, Linux, OpenVMS or Windows. There is also an
example of a .NET Console Application client.
