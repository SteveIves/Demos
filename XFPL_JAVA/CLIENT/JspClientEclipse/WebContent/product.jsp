<%@page contentType="text/html" pageEncoding="UTF-8" import="java.util.ArrayList,java.text.DateFormat,java.text.NumberFormat,SynergyApp.*" %>
...
<html>
    <head>
		...
		<%
			...
			if (product.getRelease_date().getTime().getYear()>0)
			{
				//releaseDate = df.format(product.getRelease_date().getTime());
				//
				java.util.Date dt = product.getRelease_date().getTime();
				releaseDate = df.format(dt);
			}
		
			String lastSaleDate="Unknown";
			//if (product.getLast_sale().getYear()>0)
			if (product.getLast_sale().getTime().getYear()>0)
			{
				lastSaleDate = df.format(product.getLast_sale().getTime());
			}
		%>
    </head>
		    ...
		    <tr>
		      ...
		      <td class="data">
				<%
					...
					ArrayList<Product_group> ProductGroups = (ArrayList<Product_group>)session.getAttribute("GROUPS");
					for (cnt=0; cnt<ProductGroups.size(); cnt++)
					{
						if (product.getGroup().equals(((Product_group)ProductGroups.get(cnt)).getName()))
						{
							out.print("<a href='products.jsp?group=" + ((SynergyApp.Product_group)ProductGroups.get(cnt)).getName() + "'>");
							out.print(((Product_group)ProductGroups.get(cnt)).getDescription());
							out.print("</a>");
						}
					}
				%>
