<%@page contentType="text/html" pageEncoding="UTF-8" import="java.util.ArrayList,java.util.Date,java.text.DateFormat,SynergyApp.*" %>
...
<html>
    <head>
		...
		<%
			...
			ArrayList<Order_header> order = null;
			int orderCount=0;
			...
			if (session.getAttribute("ORDERS")!=null) {
		        order = (ArrayList<Order_header>)session.getAttribute("ORDERS");
				orderCount = order.size();
				usingCache = true;
			}
			else {
		
				SynergyApp.User CurrentUser;
				String customerCode;
		
				CurrentUser = (SynergyApp.User)session.getAttribute("USER");
				customerCode = CurrentUser.getUser_customer();
		
				order = new ArrayList<SynergyApp.Order_header>();
				orderCount = synergy.GetCustomerOrders(customerCode,order);
    ...
    <body link="blue" vlink="blue" alink="blue">
		...
		<%
			...
			Order_header thisOrder;
		
			for (cnt=0; cnt<orderCount; cnt++) {
		
				thisOrder = (Order_header)order.get(cnt);
		
				orderDate = df.format(thisOrder.getOrder_date().getTime());
		
				shipDate = "";
				//if (thisOrder.getShip_date().getYear()>0) {
				if (thisOrder.getShip_date().getTime().getYear()>0) {
					shipDate = df.format(thisOrder.getShip_date().getTime());
				}
		
				deliveryDate = "";
				//if (thisOrder.getDelivery_date().getYear()>0) {
				if (thisOrder.getDelivery_date().getTime().getYear()>0) {
					deliveryDate = df.format(thisOrder.getDelivery_date().getTime());
				}
